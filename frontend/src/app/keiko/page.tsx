    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          <h1 className="text-2xl font-bold text-slate-900">El Patrón Keiko</h1>
      ...
      <div>
        ...
        <p className="text-slate-500 text-sm mb-6">
          Fujimori (17.19%) vs Sánchez Palomino (12.04%) · Votos disponibles de candidatos eliminados
        </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-5">
            ...
            <p className="text-xs text-slate-400 mb-4">
              % de votos válidos proyectados para Fujimori · Línea naranja = 50%
            </p>
            ...
            {scenarios.map((s, i) => (
              <div key={i} className="border border-slate-200 rounded-lg p-3 space-y-2">
                <div className="flex justify-between items-start">
                  ...
                  >
                    {s.projection.keiko_pct > 50 ? "Fujimori gana" : "Sánchez gana"}
                  </Badge>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 mt-1">
                  <div className="bg-slate-50 rounded p-2 border border-slate-100">
                    ...
                    <p className="text-orange-600 font-semibold text-sm">{s.projection.keiko_pct.toFixed(2)}%</p>
                    <p className="text-slate-400 text-xs">{(s.projection.keiko / 1_000_000).toFixed(2)}M votos</p>
                  </div>
                  <div className="bg-slate-50 rounded p-2 border border-slate-100">
                    <p className="text-slate-400 text-xs">Sánchez</p>
                    <p className="text-blue-600 font-semibold text-sm">{s.projection.sanchez_pct.toFixed(2)}%</p>
                    <p className="text-slate-400 text-xs">{(s.projection.sanchez / 1_000_000).toFixed(2)}M votos</p>
                  </div>
                </div>
              </div>
            ))}
