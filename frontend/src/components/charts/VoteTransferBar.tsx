    ...
    <ResponsiveContainer width="100%" height={220}>
      <BarChart data={data} layout="vertical" margin={{ left: 0, right: 60, top: 0, bottom: 0 }}>
          ...
          content={({ active, payload }) => {
            ...
            return (
              <div className="bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm space-y-1 shadow-md">
                ...
                <p className="text-orange-600">Fujimori: {d.keiko.toFixed(2)}%</p>
                <p className="text-blue-600">Sánchez: {d.sanchez.toFixed(2)}%</p>
                <p className={d.keiko_wins ? "text-green-700" : "text-red-600"}>
                  {d.keiko_wins ? "Fujimori ganaría" : "Sánchez ganaría"}
                </p>
              </div>
            )
          }}
        ...
        <Bar dataKey="keiko" radius={[0, 4, 4, 0]}
          label={{ position: "right", formatter: (v: unknown) => typeof v === "number" ? `${v.toFixed(1)}%` : "", fill: "#71717a", fontSize: 11 }}>
          {data.map((entry, i) => (
