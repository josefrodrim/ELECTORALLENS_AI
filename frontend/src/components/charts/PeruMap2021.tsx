...
interface DeptInfo {
  winner: CandCode
  margin: number
  castilloPct: number | null
  fujimoriPct: number | null
}
...
export default function PeruMap2021({ items }: Props) {
  const containerRef = useRef<HTMLDivElement>(null)
  const [active, setActive] = useState<CandCode | null>(null)
  const [tooltip, setTooltip] = useState<TooltipState | null>(null)

  // Build dept lookup by normalized name
  const deptMap: Record<string, DeptInfo> = {}
  for (const item of items) {
    const key = norm(item.geo_name)
    const winner = (item.leading_candidate_code ?? "F2") as CandCode
    const c1 = item.candidates.find((c) => c.candidate_code === "F1")
    const c2 = item.candidates.find((c) => c.candidate_code === "F2")
    deptMap[key] = {
      winner,
      margin: item.winning_margin_pct ?? 0,
      castilloPct: c1?.vote_pct ?? null,
      fujimoriPct: c2?.vote_pct ?? null,
    }
  }

  const counts = {
    F1: Object.values(deptMap).filter((d) => d.winner === "F1").length,
    F2: Object.values(deptMap).filter((d) => d.winner === "F2").length,
  }

  function getFill(geoName: string): string {
    const info = deptMap[norm(geoName)]
    if (!info) return "#e2e8f0"
    const cand = CANDS.find((c) => c.code === info.winner)!
    ...
    <div ref={containerRef} className="relative flex flex-col lg:flex-row gap-6 items-start">
      ...
      <div className="flex lg:flex-col flex-row gap-3 lg:w-52 w-full flex-wrap">
        ...
        <div className="w-full lg:w-auto rounded-xl border border-slate-100 bg-slate-50 p-3 space-y-1.5">
          <p className="text-xs text-slate-400 uppercase tracking-widest mb-2">Resultado P2</p>
          {CANDS.map((c) => (
      ...
      {tooltip && (
        ...
        >
          ...
          {(() => {
            const winner = CANDS.find((c) => c.code === tooltip.info.winner)!
            return (
              <>
                ...
                <div className="space-y-0.5 mt-1.5">
                  {CANDS.map((c) => {
                    const pct = c.code === "F1" ? tooltip.info.castilloPct : tooltip.info.fujimoriPct
                    return (
                      <div key={c.code} className="flex justify-between text-xs">
                        <span className="text-slate-500">{c.short}</span>
                        <span className="font-mono text-slate-700">{pct != null ? `${pct.toFixed(1)}%` : "—"}</span>
                      </div>
                    )
                  })}
                </div>
